  ...
  methods: {
    ...
    async decodeXSLXFile(file) {
      try {
        ...
        // get Connection Alias sheet
        let aliasSheets = XLSX.utils.sheet_to_json(workbook.Sheets["Aliases"], {
          raw: true,
          blankrows: true,
        });

        // combine all node informations from all other sheets
        let tempNodes = [];
        for (
          let sheetIndex = 0;
          ...
          // skip alias file, there are no node informations
          if (sheetName == "Aliases") continue;
          ...
          tempNodes.push(...nodes.nodes);
        }
        tempNodes = this.removeDuplicates(tempNodes);

        // map node datasets to sheets
        for (
          let sheetIndex = 0;
          sheetIndex < aliasSheets.length;
          sheetIndex++
        ) {
          // rename name key to lowercase for compatibility with other functions
          aliasSheets[sheetIndex].name = aliasSheets[sheetIndex].Name;
          delete aliasSheets[sheetIndex].Name;

          // filter nodes with same name alias
          const nodes = tempNodes.filter((node, idx, nodes) => {
            return node["Alias Name"] == aliasSheets[sheetIndex].name;
          });

          // add filtered nodes to sheed data
          aliasSheets[sheetIndex].nodes = nodes;
        }
